...
class AmazonDebugInfoClass {
	public static function generate_server_data($send = false, $main_debug = false) {
		...
		$single_only 		= (bool)get_option('appip_show_single_only');
		...
		$debug_info         = array(
			...
			'API Request Method' 		  => ((int)$apip_getmethod == 1 ? 'CURL' :'fopen' ). ' (`wp_remote_request` v3.7.1+)',
		...
		do {
			...
			if ( isset($_REQUEST['appip_debug_submit']) || isset($_REQUEST['appip_debug_submit_all']) || isset($_REQUEST['appip_debug_submit_dev']) ) {
				...
				if ( $email ) {
					$send = true;
					if($allemail){
						$to = 'amazon-plugin-help@fischercreativemedia.com;'.$email;
						$totxt = 'amazon-plugin-help@fischercreativemedia.com and '.$email;
					}else{
						$to = $email;
						$totxt = $email;
					}
					if ( wp_mail( $to , sprintf( __( 'APPIP Debug Mail From Site %s.', 'amazon-product-in-a-post-plugin' ), $siteurl ), $mail_text, '', array() ) ) {
					...
					$send = true;
					if ( wp_mail( 'amazon-plugin-help@fischercreativemedia.com', sprintf( __( 'APPIP Debug Mail From Site %s.', 'amazon-product-in-a-post-plugin' ), $siteurl ), $mail_text, '', array() ) ) {
						wp_redirect( admin_url('admin.php?page=apipp_plugin_admin&notice_sent=2&sent='.urlencode('amazon-plugin-help@fischercreativemedia.com')));
						exit;
					} else {

<?php
	class newExtFeatImgInc{
		function __construct(){
			...
			add_action( 'add_meta_boxes', array( $this, 'amazon_featured_add_url_metabox') );
			add_action( 'save_post', array( $this, 'amazon_featured_save_url') );
		...
		function amazon_featured_url_metabox( $post ) {
			...
			<a id="amazon_featured_remove_button" href="#" onClick="amazon_featuredRemoveFeaturedImage();" style="<?php echo $show_if_img; ?>">Remove featured image</a>
			<script>
			function amazon_featuredRemoveFeaturedImage() {
			...
			function amazon_featuredPreview() {
			...
			<div style="text-align:right;margin-top:10px;<?php echo $hide_if_img; ?>"><a class="button" id="amazon_featured_preview_button" onClick="amazon_featuredPreview();">Preview</a></div>
			<?php
		}
		
		function amazon_featured_save_url( $post_ID ) {
			if ( isset( $_POST['amazon_featured_url'] ) ) {
				$url = strip_tags( $_POST['amazon_featured_url'] );
				update_post_meta( $post_ID, $this->_amazon_featured_url(), $url );
			}
